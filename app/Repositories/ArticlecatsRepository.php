...
class ArticlecatsRepository extends BaseRepository
{
    ...
    public function  getCacheCategoryAll(){
            return Cache::remember('zcjy_category_all', Config::get('web.cachetime'), function(){
            try {
     ...
     * @return [type]            [description]
     */
    public function getAllCachePostsWithHot($hot=null,$skip = 0,$take = 8){
        return Cache::remember('zcjy_posts_of_category_all_desc_'.$skip.'_'.$take, Config::get('web.cachetime'), function() use ($hot,$skip,$take) {
            try {
                $posts = collect([]);
                if(empty($hot)){
                    $posts = Post::orderBy('created_at','desc')->where('status', 1)->skip($skip)->take($take)
                            ->with('admin')
                            ->with('user')
                            ->with('products.specs')
                            ->get();
                }else{
                    $posts = Post::orderBy('created_at','desc')->where('status', 1)->where('is_hot',1)->skip($skip)->take($take)
                            ->with('admin')
                            ->with('user')
                            ->with('products.specs')
                            ->get();
                ...
                foreach ($posts as $key => $value) {
                    ...
                    $value['images'] = get_content_img($value->content);
                    $value['sub_content'] = des($value->content,100);
                }
     ...
     * @return [type]            [description]
     */
    public function getCachePostsOfCatIdWithHot($hot=null, $cat_id, $skip = 0, $take = 8){
        return Cache::remember('zcjy_posts_of_category_id_desc_'.$hot.'_'.$cat_id.'_'.$skip.'_'.$take, Config::get('web.cachetime'), function() use ($hot,$cat_id, $skip, $take) {
            try {
                $posts = collect([]);
                $cat = $this->getCacheCategory($cat_id);
                if ($cat) {
                    if(empty($hot)){
                        $posts = $cat->posts()->where('status', 1)->orderBy('created_at','desc')->skip($skip)->take($take)
                            ->with('admin')
                            ->with('user')
                            ->with('products.specs')
                            ->get();
                    }else{
                        $posts = $cat->posts()->where('status', 1)->orderBy('created_at','desc')->where('is_hot',1)->skip($skip)->take($take)
                            ->with('admin')
                            ->with('user')
                            ->with('products.specs')
                            ->get();
