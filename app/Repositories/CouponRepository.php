...
class CouponRepository extends BaseRepository
{
    ...
    public function getCouponProductListByCouponId($coupon_id){
        $coupon=Coupon::find($coupon_id);
        if(!empty($coupon)) {
            return $coupon->products()->with('specs')->get();
        }else{
            return [];
        }
    }
     ...
     */
    public function couponCanUse($user)
    {
        $today = \Carbon\Carbon::today();
        return $user->coupons()->where('status', 0)->where('time_begin', '<=', $today)->where('time_end', '>=', $today)->get();
    }
