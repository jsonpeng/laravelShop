            ...
            <table class="table table-responsive" id="products-table">
                ...
                @if(count($items)>0)
                    ...
                    <tr data-productid="{!! $item->
                        ...
                        product()->first()->remark !!}" data-weblink="{{ route('front.mobile.product',$item->
                        product()->first()->id) }}" data-minilink="../product/product?product_id={{ $item->
                        product()->first()->id }}">
                        <td></td>
                        <td>
                ...
                @else
                    <tr data-productid="{!! $item->
                        id !!}" data-specid="0" data-productname="{!! $item->name !!}" data-price="{!! $item->price !!}" data-keyname="--" data-productimg="{!! $item->image !!}"  data-inventory="{!! $item->inventory !!}" data-prom="{!! empty($item->prom_type)?'false':'true' !!}" data-remark="{!! $item->remark !!}" data-weblink="{{ route('front.mobile.product',$item->id) }}" data-minilink="../product/product?product_id={{ $item->id }}">
                        <td></td>
                        <td>
                            @if(!empty($item->prom_type)) <strong style="color: red">[ @if($item->prom_type=='1')秒杀抢购中@endif @if($item->prom_type==2)团购中@endif @if($item->prom_type==3)促销中@endif @if($item->prom_type==4)订单促销中@endif @if($item->prom_type==5)拼团中@endif ]</strong> 
                            @endif {!! $item->name !!}
