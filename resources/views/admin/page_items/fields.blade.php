...
<div class="form-group col-sm-12">
    {!! Form::label('type', '表单类型:') !!}
     <select class="form-control"  name="type" >
        ...
        <option  value="textarea"   @if(!empty($pageItems))@if($pageItems->type=="textarea") selected @endif @endif>textarea(适用于多行文本,不可添加图片)</option>
        <option  value="number"   @if(!empty($pageItems))@if($pageItems->type=="number") selected @endif @endif>number(适用于简单的数字)</option>
        <option  value="longText"   @if(!empty($pageItems))@if($pageItems->type=="longText") selected @endif @endif>longText(适用于多行文本,可添加图片)</option>
  </select>  
</div>
