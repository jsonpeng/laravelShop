        ...
        <div class="row">
            ...
            <div class="col-sm-9 col-lg-10">
                <section class="content-header">
                    <h1 class="pull-left">团购列表</h1>
                    <h1 class="pull-right">
                        <a class="btn btn-primary pull-right" style="margin-top: -10px;margin-bottom: 5px" href="{!! route('teamSales.create') !!}">添加</a>
                    </h1>
                </section>
                <div class="content pdall0-xs">
                        ...
                        <div class="box-body">
                            <form id="order_search">
                                <div class="form-group col-md-2 col-sm-6 col-xs-6">
                                    ...
                                    <input type="text" class="form-control" name="name" placeholder="名称" @if (array_key_exists('name', $input))value="{{$input['name']}}"@endif></div>

                                <div class="form-group col-md-2 col-sm-6 col-xs-6">
                                    <label for="recommend">活动类型</label>
                                    <select class="form-control" name="type">
                                        <option value="" @if (!array_key_exists('type', $input)) selected="selected" @endif>全部</option>
                                        <option value="0" @if (array_key_exists('type', $input) && $input['type'] == '0') selected="selected" @endif>分享团</option>
                                        <option value="1" @if (array_key_exists('type', $input) && $input['type'] == '1') selected="selected" @endif>佣金团</option>
                                        <option value="2" @if (array_key_exists('type', $input) && $input['type'] == '2') selected="selected" @endif>抽奖团</option>
                                    </select>
                                </div>
                  ...
                  {{--               <div class="form-group col-md-2 col-sm-6 col-xs-12">
                    ...
                    <div class="box box-primary">
                        <div class="box-body">@include('admin.team_sales.table')</div>
                    </div>
                    <div class="text-center">
                        <div class="tc">
                            <?php echo $teamSales->appends($input)->render(); ?></div>
                    </div>
                </div>
            </div>
        </div>
